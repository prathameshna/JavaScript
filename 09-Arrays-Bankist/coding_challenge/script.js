...
const calcAverageHumanAge = function (ages) {
    const humaneAges = ages.map(age => age <= 2 ? age * 2 : 16 + age * 4);
    const adults = humaneAges.filter(age => age >= 18);
    console.log(humaneAges);
    console.log(adults);

    const avgAdultsAge = adults.reduce((acc, age) => acc + age, 0) / adults.length;
    // const avgAdultsAge = adults.reduce((acc, age, i, arr) => acc + age / arr.length, 0);
...
const checkDogEatingOkay = dog => 
    dog.curFood > (dog.reccFood * 0.90) && dog.curFood < (dog.reccFood * 1.10);
